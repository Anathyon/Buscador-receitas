  ...
  return (
    <header className="bg-[#aa550f] shadow-md w-full flex justify-between items-center sm:px-8 fixed top-0 z-50" style={{paddingInline:"1rem", paddingBlock:"0.75rem"}}>
      ...
      <div className="hidden md:flex items-center space-x-8">
        <nav style={{ gap:"0.5rem"}} className="flex items-center space-x-6">
          <Link to="/" className="flex items-center rounded-full font-semibold transition-colors duration-200 bg-[#ffedd5] text-[#d97706] hover:bg-[#d97706] hover:text-[#ffedd5] shadow-sm"
               style={{paddingInline:"1.5rem", paddingBlock:"0.5rem",}}
          >
            <FaHome style={{ marginRight: '0.5rem' }} />
            <FormattedMessage id="header.home" />
          </Link>
          <Link to="/buscar" style={{paddingInline:"1rem", paddingBlock:"0.5rem"}} className="flex items-center rounded-full font-semibold text-white border-2 border-white hover:bg-amber-900 transition-colors duration-200">
        ...
        <div className="relative" style={{marginLeft:"1rem", marginRight:"0.5rem"}}>
          <button onClick={toggleLanguageSelector} style={{paddingInline:"1rem", paddingBlock:"0.5rem", gap:"0.75rem"}} className="flex items-center justify-center space-x-2 w-24 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-300">
          ...
          {isLanguageSelectorOpen && (
            <div className="absolute top-14 right-0 w-40 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden" style={{padding:"0.5rem"}}>
              {languageOptions.map(({ code, labelId }) => (
                <button key={code} onClick={() => handleLanguageChange(code)} style={{padding:"0.5rem"}} className={`flex items-center w-full text-left rounded-md transition-colors duration-200 ${locale === code ? 'bg-orange-100 text-orange-600 font-bold' : 'text-gray-700 hover:bg-gray-100'}`}>
